...
- [x] 1.1 Update `settings-panel` to define the Tauri Settings entrypoint, the tabbed information architecture, and the minimal polished scope for persistent viewer preferences.
...
- [x] 2.1 Add a native Settings menu item / shortcut in the Tauri shell that opens or focuses a dedicated settings window.
- [x] 2.2 Implement a lightweight settings window with Behavior and Appearance tabs, separate from the viewer viewport.
- [x] 2.3 Keep the settings window visually polished but minimal, with stronger grouping, labels, and hierarchy than the current functional baseline.
...
- [x] 3.2 Implement real native opacity editing for supported platforms, plus safe bounds and unsupported-platform fallback for blur.
- [x] 3.3 Preserve manual Fit as an action-only command rather than reintroducing a persisted fit toggle.
- [x] 3.4 Isolate active image state per viewer window so one window cannot overwrite another.
